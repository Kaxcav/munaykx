  ...
  return (
    <>
      ...
      <Pagina
        ...
        // Mesma composição de eyebrow da `/comunidades/[slug]`: a etiqueta do
        // conteúdo e, quando é ilustrativo, o selo "exemplo" (regra 3) junto.
        eyebrow={
          <>
            Evento · {evento.community.modalidade} · {evento.community.regiao}
            {evento.demo && <Badge variant="outline">exemplo</Badge>}
          </>
        }
      ...
      >
        ...
        {jaPassou ? (
          <Secao titulo="Esse evento já aconteceu" destaque className="max-w-3xl">
            <Card className="p-8">
          ...
          <Secao titulo="Confirmar presença" className="max-w-3xl">
            {/*
              Chamada em corpo display, não em `<h2>`: o cabeçalho da seção já é
              o `<h2>` do `<Secao>`, e dois títulos de mesmo nível empilhados
              quebrariam o esqueleto da página pro leitor de tela. A tipografia
              é a mesma de antes.
            */}
            <p className="mb-6 font-display text-2xl font-extrabold tracking-tight">
